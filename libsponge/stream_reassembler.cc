...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
    ...
    if (index <= first_unassembled) {
        if (index + data.length() < first_unassembled)
            ...
            while (!_buf.empty() && _buf.begin()->first + _buf.begin()->second.length() <= first_unassembled)
                _buf.pop_front();
    ...
    } else if (index >= first_unacceptalbe)
        return;
    else {
        if (data.length() == 0)
            return;
        ...
        if (_buf.empty()) {
            _buf.push_back({index, ndata});
            return;
        }
        auto it = find_before(index);
        if (it == _buf.end())
            it = _buf.insert(_buf.begin(), {index, ndata});
        else if (it->first + it->second.length() >= index) {
            if (it->first + it->second.length() >= index + ndata.length())
                return;
